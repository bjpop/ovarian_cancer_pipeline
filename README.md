...
ovarian_cancer_pipeline is based on the [Ruffus](http://www.ruffus.org.uk/) library for writing bioinformatics pipelines. Its features include:
...
`ovarian_cancer_pipeline` depends on the following programs and libraries:
...
```
cd /place/to/install
virtualenv ovarian_cancer_pipeline
source ovarian_cancer_pipeline/bin/activate
pip install -U https://github.com/bjpop/ovarian_cancer_pipeline
```
...
git clone https://github.com/bjpop/ovarian_cancer_pipeline
```

#### Install `ovarian_cancer_pipeline` as described above

#### Get a reference genome.

```
cd ovarian_cancer_pipeline/example
mkdir reference
...
#### Run `ovarian_cancer_pipeline` and ask it what it will do next

```
ovarian_cancer_pipeline -n --verbose 3
```

#### Generate a flowchart diagram

```
ovarian_cancer_pipeline --flowchart pipeline_flow.png --flowchart_format png
```

#### Run the pipeline

```
ovarian_cancer_pipeline --use_threads --log_file pipeline.log --jobs 2 --verbose 3
```
...
```
ovarian_cancer_pipeline -h
usage: ovarian_cancer_pipeline [-h] [--verbose [VERBOSE]] [-L FILE] [-T JOBNAME] [-j N]
